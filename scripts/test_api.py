...
API_KEY = "ts_live_95edd6b67e00b1f099c5fe2ea3362441d345d8a2dec870aa"
...
NORMAL_ENDPOINTS = [
    "/health",
    "/api/profile",
    "/api/users/me",    # safe: /api/user prefix match now requires /api/user/ or exact /api/user
    "/api/products",
    "/api/search",
    "/api/feed",
]
        ...
        while not stop_event.is_set():
            try:
                ...
                if response.status_code == 429:
                    print(f"\n⚠️  Rate limited: User {user_id} hit {endpoint}")
                elif response.status_code == 500:
                    print(f"\n❌ Server error: User {user_id} hit {endpoint}")
...
class Simulation:
    ...
    async def run(self):
        ...
        print(f"🚫 Rate limited requests: {self.rate_limited_count}")
